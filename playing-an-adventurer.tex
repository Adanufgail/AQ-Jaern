...
This area of effect is defined by drawing a line from the caster's finger tip \tcdefine{Y units} toward the spell target. All objects within a column whose radius is one half of the width (\tcdefine{X/2 units}) can be affected by the spell. Intervening objects within the area may partially or fully shield other objects from the spell effect (GM's discretion). Once the spell is activated, the GM may use normal laws of physics to determine how the effect acts, if it's physical in nature.
\section{Objects}
